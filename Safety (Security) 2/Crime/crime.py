...
queries=["HOMIC_RA","VEH_THEFT_RA"]
titles=["Intentional Homicide","Motor Vehicle Theft"]
labels=["Homicide Rate (per 100.000 population)","Motor Vehicle Theft Rate (per 10.000 population)"]
...
for q in range(2):
    ...
    for i in range(len(countries)):
            country=countries[i]
            countr_filtered_df = dataframe.loc[(dataframe['REG_ID']==country)]#all data beloning to that country
            country_df=getFromFiltered(countr_filtered_df,"VAR",queries[q])
...
fig0.savefig("crime_data"+".jpg", dpi=500)
fig0.show()
