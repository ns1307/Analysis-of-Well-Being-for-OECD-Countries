...
no_data_countries= []
new_colors=[]
filtered_df = dataframe.loc[(dataframe['VAR']=="ROOMS_PC")]
for i in range(len(countries)):
    country=countries[i]
    country_df = filtered_df.loc[(filtered_df['REG_ID']==country)]#all data beloning to that country
...
plt.xlabel("Countries")
plt.ylabel("Number of Rooms per inhabitant")
title="Average number of rooms per inhabitant for OECD Countries"
plt.title(title)
...
plt.savefig("number of rooms per inhabitant"+".jpg", dpi=500)
plt.show()
