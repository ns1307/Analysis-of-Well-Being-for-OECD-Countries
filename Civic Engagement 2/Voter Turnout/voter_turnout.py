...
dataframe= pd.read_csv("voter_turnout.csv")
...
no_data_countries= []
new_colors=[]
filtered_df = dataframe.loc[(dataframe['VAR']=="VOTERS_RA")]
for i in range(len(countries)):
    country=countries[i]
    country_df = filtered_df.loc[(filtered_df['REG_ID']==countries[i])]#all data beloning to that country
...
title="Similarity Matrix Voter Turnout"
plt.title(title)
...
title="Number of Comparisons Voter Turnout"
plt.title(title)
plt.xlabel("Countries")
plt.ylabel("Countries")
plt.savefig(title+".png", dpi=500)
...
plt.title('Voters Turnout to General Elections (in % of registered voters who voted)')
...
plt.savefig("voter_turnout_chart"+".jpg", dpi=500)
plt.show()
