...
dataframe= pd.read_csv("env_politics_index.csv")
...
filtered_df = dataframe.loc[(dataframe['VAR']=="EPS")]


years=list(np.arange(1990,2021))
...
for i in range(len(countries)):
 ...
 """
    ...
    title="Environmental Policy Stringency Index (EPS)  for "+ country
    plt.title(title)
    plt.xlabel("Year")
    plt.ylabel("Stringency Index (0-6)")
    plt.tight_layout()
