...
for i in range(len(countries)):
    ...
    plt.legend(["Total emplpyed","Employed women","Employed men"])
    text="R2 score:"+str(round(r2_score(tot_values, mymodel(X)),2))
    plt.figtext(0,0, text, ha="left", fontsize=10)
    
    plt.plot(myline, mymodel(myline), c="GREY")
    plt.legend(["Total emplpyed","Employed women","Employed men","Employment Rate Prediction"])

    title="Employment Rate Chart for "+ country
    plt.title(title)
    plt.xlabel("Year")
    plt.ylabel("Employment of Workable Population (%)")
    plt.tight_layout()
