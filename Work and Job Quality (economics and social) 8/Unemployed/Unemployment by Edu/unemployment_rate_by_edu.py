...
for i in range(3):
    non_NA_countries=get_non_na_countries(countries,unemployment_table, i)
    ...
    countries=unemployment_table.index.values.tolist()
...
for i in range(len(countries)):
    ...
    if len(bupps_values_unemp)>0:
        plt.scatter(bupps_years_unemp, bupps_values_unemp,c="maroon")
        legend.append(ed_levels[0])
        mymodel = np.poly1d(np.polyfit(bupps_years_unemp, bupps_values_unemp, best_degree))
        myline = np.linspace(bupps_years_unemp[0]-1,bupps_years_unemp[-1]+2)
        plt.plot(myline, mymodel(myline), c="orangered")
        legend.append("Prediction for "+ ed_levels[0])
        text1="R2 score for "+ed_levels[0]+": "+str(round(r2_score(bupps_values_unemp, mymodel(bupps_years_unemp)),2))
        
    if len(upps_values_unemp)>0:
        plt.scatter(upps_years_unemp, upps_values_unemp,c="navy")
        legend.append(ed_levels[1])
        mymodel = np.poly1d(np.polyfit(upps_years_unemp, upps_values_unemp, best_degree))
        myline = np.linspace(upps_years_unemp[0]-1,upps_years_unemp[-1]+2)
        plt.plot(myline, mymodel(myline), c="royalblue")
        legend.append("Prediction for "+ ed_levels[1])
        text2="R2 score for "+ed_levels[1]+": "+str(round(r2_score(upps_values_unemp, mymodel(upps_years_unemp)),2))
        
    if len(tri_values_unemp)>0:
        plt.scatter(tri_years_unemp , tri_values_unemp,c="green")
        legend.append(ed_levels[2])
        mymodel = np.poly1d(np.polyfit(tri_years_unemp, tri_values_unemp, best_degree))
        myline = np.linspace(tri_years_unemp[0]-1,tri_years_unemp[-1]+2)
        plt.plot(myline, mymodel(myline), c="lime")
        legend.append("Prediction for "+ ed_levels[2])
        text3="R2 score for "+ed_levels[2]+": "+str(round(r2_score(tri_values_unemp, mymodel(tri_years_unemp)),2))
    text=text1+"\n"+text2+"\n"+text3
    plt.figtext(0,0, text, ha="left", fontsize=7)
    
    plt.legend(legend,prop={'size': 6})
   
    title="Unemployment Rates by Education Levels for "+ country
    plt.title(title)
    plt.xlabel("Year")
    plt.ylabel("Unemployment Percentage by Edu Level (%)")
    plt.tight_layout()
